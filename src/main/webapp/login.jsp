...
<c:set var="ignoredQueryParams"
       value=",loginOp,loginNewPassword,totpcode,loginConfirmNewPassword,loginErrorCode,username,email,password,zrememberme,ztrusteddevice,zlastserver,client,login_csrf,"/>
...
<c:if test="${not empty param.username and not empty param.virtualacctdomain}">
    ...
    <c:set var="trimmedUserName"
           value="${fn:replace(param.username,'@' ,'.')}@${param.virtualacctdomain}"/>
</c:if>

<c:if test="${not empty trimmedUserName}">
    <c:choose>
        <c:when test="${(fn:indexOf(trimmedUserName,'@') == -1) and !(empty param.customerDomain)}">
            <c:set var="fullUserName"
                   value="${trimmedUserName}@${param.customerDomain}"/>
        </c:when>
        <c:otherwise>
...
<c:catch var="loginException">
    <c:choose>
        ...
        <c:when test="${param.loginOp eq 'relogin' and not empty param.loginErrorCode}">
            ...
            <fmt:message bundle="${zmsg}" var="errorMessage"
                         key="${errorCode}"/>
        ...
        <c:when test="${param.loginOp eq 'logout'}">
            <zm:getDomainInfo var="domainInfo" by="virtualHostname"
                              value="${zm:getServerName(pageContext)}"/>
            <c:set var="logoutRedirectUrl"
                   value="${domainInfo.attrs.zimbraWebClientLogoutURL}"/>
            <c:set var="isAllowedUA"
                   value="${zm:isAllowedUA(ua, domainInfo.webClientLogoutURLAllowedUA)}"/>
            <c:set var="isAllowedIP"
                   value="${zm:isAllowedIP(remoteAddr, domainInfo.webClientLogoutURLAllowedIP)}"/>
            <c:choose>
                ...
                <c:when test="${!empty cookie.ZM_TEST}">
                    ...
                    <c:choose>
                        <c:when test="${(not empty param.login_csrf) && (param.login_csrf eq cookie.ZM_LOGIN_CSRF.value)}">
                            <zm:login username="${fullUserName}"
                                      password="${param.password}"
                                      varRedirectUrl="postLoginUrl"
        ...
        <c:otherwise>
            ...
            <c:set var="authtoken"
                   value="${not empty param.zauthtoken ? param.zauthtoken : cookie.ZM_AUTH_TOKEN.value}"/>
            <c:if test="${not empty authtoken}">
                <zm:login authtoken="${authtoken}"
                          authtokenInUrl="${not empty param.zauthtoken}"
                          twoFactorCode="${not empty param.totpcode ? param.totpcode : ''}"
                          varRedirectUrl="postLoginUrl"
                          varAuthResult="authResult"
                          rememberme="${param.zrememberme == '1'}"
                          trustedDevice="${param.ztrusteddevice == 1}"
                          requestedSkin="${param.skin}"
                          adminPreAuth="${param.adminPreAuth == '1'}"
                          importData="true" csrfTokenSecured="true"
                          attrs="zimbraFeatureConversationsEnabled"/>
            ...
            <c:choose>
                    ...
                    <c:choose>
                        <c:when test="${not empty refer and not zm:equalsIgnoreCase(refer, serverName)}">
                            ...
                            <jsp:forward page="/h/postLoginRedirect">
                                <jsp:param name="zauthtoken"
                                           value="${authResult.authToken.value}"/>
                ...
                <c:otherwise>
                    ...
                    <c:if test="${empty client and touchSupported}">
                        <c:set var="client"
                               value="${touchLoginPageExists ? 'touch' : 'mobile'}"/>
                    </c:if>
                    ...
                    <c:if test="${empty client or client eq 'preferred'}">
                        <c:set var="client"
                               value="${requestScope.authResult.prefs.zimbraPrefClientType[0]}"/>
                    </c:if>
                    <c:choose>
                        <c:when test="${client eq 'socialfox'}">
                            <c:set var="sbURL"
                                   value="/public/launchSidebar.jsp"/>
...
<c:if test="${loginException != null}">
    ...
    <c:forEach var="arg" items="${error.arguments}">
        <fmt:message bundle="${zhmsg}" var="errorMessage"
                     key="${errorCode}.${arg.name}">
    ...
    <c:if test="${errorCode eq 'account.AUTH_FAILED' and not empty param.virtualacctdomain}">
        <fmt:message bundle="${zhmsg}" var="errorMessage"
                     key="account.EXTERNAL_AUTH_FAILED"/>
    </c:if>
...
<c:set var="loginRedirectUrl"
       value="${zm:getPreLoginRedirectUrl(pageContext, '/')}"/>
...
<c:if test="${((empty pageContext.request.queryString) or (fn:indexOf(pageContext.request.queryString,'customerDomain') == -1)) and (empty param.virtualacctdomain) and (empty virtualacctdomain) }">
    <c:set var="domainLoginRedirectUrl"
           value="${domainInfo.attrs.zimbraWebClientLoginURL}"/>
    <c:set var="isAllowedUA"
           value="${zm:isAllowedUA(ua, domainInfo.webClientLoginURLAllowedUA)}"/>
    <c:set var="isAllowedIP"
           value="${zm:isAllowedIP(remoteAddr, domainInfo.webClientLoginURLAllowedIP)}"/>
</c:if>
...
<html class="user_font_size_normal"
      lang="${fn:substring(pageContext.request.locale, 0, 2)}">
<head>
    <!--
     login.jsp
     * ***** BEGIN LICENSE BLOCK *****
    ...
    -->
    <c:set var="client" value="${param.client}"/>
    <c:set var="useStandard"
           value="${not (ua.isFirefox3up or ua.isGecko1_9up or ua.isIE9up or ua.isSafari4Up or ua.isChrome or ua.isModernIE)}"/>
    ...
    <c:set var="smallScreen"
           value="${client eq 'mobile' or client eq 'socialfox'}"/>
    ...
    <link rel="stylesheet" type="text/css"
          href="<c:url value='/css/common,login,zhtml,skin.css'>
		<c:param name="skin"    value="${skin}" />
            ...
            <form method="post" name="loginForm"
                ...
                <form method="post" name="loginForm" action="${formActionUrl}"
                    ...
                    <c:if test="${totpAuthRequired || errorCode eq 'account.TWO_FACTOR_AUTH_FAILED'}">
                        ...
                        <input type="hidden" name="zrememberme"
                               value="${param.zrememberme}"/>
                    </c:if>
                    </c:otherwise>
                    </c:choose>
                    <c:if test="${errorCode != null}">
                        <div id="ZLoginErrorPanel">
                            <table>
                                <tr>
                                    <td><app:img id="ZLoginErrorIcon"
                                                 altkey='ALT_ERROR'
                                                 src="dwt/ImgCritical_32.png"/></td>
                    ...
                    <c:choose>
                        <c:when test="${totpAuthRequired || errorCode eq 'account.TWO_FACTOR_AUTH_FAILED'}">
                            <table class="form" id="totpTable"
                                ...
                                <tbody>
                                <tr>
                                    <td><label for="totpcode"><fmt:message
                                            key="twoFactorAuthCodeLabel"/>:</label>
                                    </td>
                                    <td><input id="totpcode" class="zLoginField"
                                               name="totpcode" type="text"
                                               value=""
                                               size="40"
                                               maxlength="${domainInfo.webClientMaxInputBufferLength}"
                                ...
                                <tr style="vertical-align:top">
                                    ...
                                    <td style="padding: 0;"><input
                                            ...
                                            value="Verify"
                                            class="ZLoginButton DwtButton"></td>
                                </tr>
                                <c:if test="${authResult.trustedDevicesEnabled eq true}">
                                    <tr style="vertical-align:top">
                                        <td/>
                                        <td><input id="trustedDevice" value="1"
                                                   type="checkbox"
                                                   name="ztrusteddevice">
                                            <label for="trustedDevice"><fmt:message
                                                    key="${mobileSupported || touchSupported ? 'twoFactorAuthTrustDevice' : 'twoFactorAuthTrustComputer'}"/></label>
                                        </td>
                                    </tr>
                                </c:if>
                                </tbody>
                        ...
                        <c:otherwise>
                            <table class="LoginBoxForm">
                                <tr>
                                    <td colspan="2"><label class="LoginTr LoginTile">邮箱登录</label></td>
                                </tr>
                                <c:choose>
                                    <c:when test="${not empty domainLoginRedirectUrl && param.sso eq 1 && empty param.ignoreLoginURL && (isAllowedUA eq true)}">
                                        <tr>
                                    ...
                                    <c:otherwise>
                                            ...
                                            <tr>
                                                <td colspan="2">
                                                    <div style="position:relative;">
                                                        ...
                                                        <input placeholder="新密码" id="loginNewPassword"
                                                           autocomplete="off"
                                                           class="zLoginField"
                                                           name="loginNewPassword"
                                                           type="password"
                                                           value="${fn:escapeXml(param.loginNewPassword)}"
                                                           size="40"
                                                           maxlength="${domainInfo.webClientMaxInputBufferLength}"/>
                                                    </div>
                                                </td>
                                            ...
                                            <tr>
                                                <td colspan="2">
                                                    <div style="position:relative;">
                                                        ...
                                                        <input placeholder="确认新密码" id="confirmNew"
                                                           autocomplete="off"
                                                           class="zLoginField"
                                                           name="loginConfirmNewPassword"
                                                           type="password"
                                                           value="${fn:escapeXml(param.loginConfirmNewPassword)}"
                                                           size="40"
                                                           maxlength="${domainInfo.webClientMaxInputBufferLength}"/>
                                                    </div>
                                                </td>
                                        ...
                                        <tr>
                                            <td colspan="2" class="submitTD">
                                                <c:set var="isSignedInDisabled"
                                                       value="${domainInfo.attrs.zimbraWebClientStaySignedInDisabled}"/>
                                                <c:if test="${isSignedInDisabled eq false}">
                                                    <input id="remember"
                                                           value="1"
                                                           type="checkbox"
                                                           name="zrememberme"/>
                                                    <label for="remember"><fmt:message
                                                            key="${smallScreen?'rememberMeMobile':'rememberMe'}"/></label>
                                                </c:if>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td class="submitTD">
                                                    <a href="#"
                                                       ...
                                                       id="ZLoginForgotPassword"
                                                       aria-controls="ZLoginForgotPassword"
                                                       aria-expanded="false"><fmt:message
                                                            key="forgotPassword"/></a>
                                                </td>
                                            </tr>
                                        </c:if>
                                    </c:otherwise>
                                ...
                                <c:if test="${empty param.virtualacctdomain}">
                                    ...
                                    <tr
                                            <c:if test="${client eq 'socialfox'}">style='display:none;'</c:if>>
                                        <td>
                                            <label for="client"><fmt:message
                                                    key="versionLabel"/></label>
                                        </td>
                                        <td>
                                                    ...
                                                    <c:otherwise>
                                                        <select id="client"
                                                            ...
                                                            <option value="preferred"
                                                                    <c:if test="${client eq 'preferred'}">selected</c:if> >
                                                                <fmt:message
                                                                        key="clientPreferred"/></option>
                                                            <option value="advanced"
                                                                    <c:if test="${client eq 'advanced'}">selected</c:if>>
                                                                <fmt:message
                                                                        key="clientAdvanced"/></option>
                                                            <option value="standard"
                                                                    <c:if test="${client eq 'standard'}">selected</c:if>>
                                                                <fmt:message
                                                                        key="clientStandard"/></option>
                                                            <option value="mobile"
                                                                    <c:if test="${client eq 'mobile'}">selected</c:if>>
                                                                <fmt:message
                                                                        key="clientMobile"/></option>
                                                            <c:if test="${zimbraxSupported}">
                                                                <option value="zimbrax"
                                                                        <c:if test="${client eq 'zimbrax'}">selected</c:if>>
                                                                    <fmt:message
                                                                            key="clientZimbrax"/></option>
                                                            </c:if>
                                                            <c:if test="${touchLoginPageExists}">
                                                                <option value="touch"
                                                                        <c:if test="${client eq 'touch'}">selected</c:if>>
                                                                    <fmt:message
                                                                            key="clientTouch"/></option>
                                                            </c:if>
                                                        </select>
                                                    </c:otherwise>
                                                </c:choose>
                                                ...
                                                <c:choose>
                                                    <c:when test="${touchLoginPageExists}">
                                                        <div id="ZLoginWhatsThis"
                                                             class="ZLoginInfoMessage"
                                                             style="display:none;"
                                                             onclick='showWhatsThis();'
                                                             role="tooltip">
                                                            <fmt:message
                                                                    key="clientWhatsThisMessage"/></div>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <div id="ZLoginWhatsThis"
                                                             class="ZLoginInfoMessage"
                                                             ...
                                                             role="tooltip">
                                                            <fmt:message
                                                                    key="clientWhatsThisMessageWithoutTablet"/></div>
                                                    </c:otherwise>
                                                </c:choose>
                                                <div id="ZLoginUnsupported"
                                                     class="ZLoginInfoMessage"
                                                     style="display:none;">
                                                    <fmt:message
                                                            key="clientUnsupported"/></div>
                                            </div>
                                        </td>
                                    </tr>
                                </c:if>
...
<script>
    ...
    var resizeLoginPanel = function () {
        ...
        if (panelElem && !panelElem.style.maxWidth) {
            if (document.body.clientWidth >= 500) {
                panelElem.style.width = "500px";
            } else {
                panelElem.style.width = "90%";
            }
        }
    }
    resizeLoginPanel();
    if (window.attachEvent) {
        window.attachEvent("onresize", resizeLoginPanel);
    }
    </c:if>
